...
`MailReader` is classified as `Legacy Skeleton` under Portfolio Project #24. Preserve public/upstream compatibility, privacy, and evidence boundaries; do not infer maturity beyond executable tests.
...
This repository follows [Portfolio Project #24](https://github.com/users/googa27/projects/24) and [MailReader rollout issue](https://github.com/googa27/MailReader/issues/1). A repository-specific, evidence-backed exception in `docs/ARCHITECTURE.yaml` may specialize a rule; undocumented drift is not an exception.
...
Repository posture: Avoid core dependencies. Data posture: No current data architecture; revival requires private-email classification, retention, and local custody plan.
